...
export default function Footer() {
  return (
    <footer id="contact" className="bg-[#0a0812]">
      ...
      {/* Newsletter Section */}
      <div className="flex flex-col items-center px-16 py-20 border-b border-white/5">
        <h2 className="text-4xl font-bold text-white text-center mb-3">
          Subscribe to our newsletter
        </h2>
        <p className="text-white/50 text-sm mb-8 text-center">
          Stay updated with BGS Ecosystem — get the latest news, features, and insights.
        </p>

        {/* Email input */}
        <div className="flex items-center w-full max-w-md rounded-xl border border-white/15 bg-white/5 overflow-hidden focus-within:border-red-400/50 transition-colors duration-300">
          ...
          <input
            ...
            className="flex-1 bg-transparent py-3 pr-2 text-sm text-white placeholder:text-white/30 outline-none"
          />
          <button className="m-1 rounded-lg bg-red-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-red-600 transition-colors duration-300 shadow-[0_0_15px_rgba(239,68,68,0.4)]">
            Get started
          </button>
        </div>
        ...
        <div className="mt-5 flex items-center gap-3">
          <div className="flex -space-x-2">
            {["bg-red-400", "bg-purple-400", "bg-blue-400", "bg-green-400"].map((c, i) => (
              <div key={i} className={`w-7 h-7 rounded-full border-2 border-[#0a0812] ${c} flex items-center justify-center text-[10px] font-bold text-white`}>
                {String.fromCharCode(65 + i)}
              </div>
            ))}
          </div>
          <span className="text-xs text-white/40">Our experts are ready to help!</span>
        </div>
      </div>
      ...
      <div className="px-16 py-10">
        <div className="relative rounded-3xl overflow-hidden" style={{ background: "linear-gradient(135deg, #1a0a2e 0%, #0d0820 60%, #1a1030 100%)" }}>
          {/* Globe visual */}
          <div className="absolute right-0 top-0 bottom-0 w-96 opacity-60">
          ...
          <div className="relative z-10 px-14 py-14 max-w-lg">
      ...
      {/* Main Footer Links */}
      <div className="px-16 py-14 grid grid-cols-[2fr_1fr_1fr_1fr] gap-12 border-t border-white/5">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-semibold text-white">hello@bgsecosystem.com</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Bottom bar */}
      <div className="px-16 py-5 border-t border-white/5 flex items-center justify-center">
